...
import { createReview, updateReview, deleteReview, getReviewById, getMyBookmarkedReviews, setReviewLike, setReviewBookmark } from '../../api/review';
...
export function useCreateReviewMutation() {
  ...
  return useMutation({ mutationFn: createReview, onSuccess: () => client.invalidateQueries({ queryKey: queryKeys.reviews.all }) });
}
export function useUpdateReviewMutation() {
  const client = useQueryClient();
  return useMutation({
    mutationFn: ({ id, payload }: { id: string | number; payload: PostUpdateRequestDto }) => updateReview(id, payload),
...
export function useReviewLikeMutation() {
  const client = useQueryClient();
  return useMutation({
    mutationFn: ({ id, value }: { id: string | number; value: boolean }) => setReviewLike(id, value),
    onSuccess: () => client.invalidateQueries({ queryKey: queryKeys.reviews.all }),
  });
}
export function useReviewBookmarkMutation() {
  const client = useQueryClient();
  return useMutation({
    mutationFn: ({ id, value }: { id: string | number; value: boolean }) => setReviewBookmark(id, value),
    onSuccess: () => client.invalidateQueries({ queryKey: queryKeys.reviews.all }),
  });
}
