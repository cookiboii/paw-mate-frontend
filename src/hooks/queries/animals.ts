...
import { fetchAllAnimals, fetchAnimalList, fetchAnimalListBySpecies, fetchAnimalById, deleteAnimal, updateAnimalStatus } from '../../api/animal';
...
export const useAnimalListQuery = (page: number, size: number, species = 'ALL', enabled = true) =>
  useQuery({ queryKey: queryKeys.animals.list(page, size, species), queryFn: () => species === 'ALL' ? fetchAnimalList(page, size) : fetchAnimalListBySpecies(species, page, size), enabled });
export const useAllAnimalsQuery = (enabled = true, species = 'ALL') =>
  useQuery({ queryKey: queryKeys.animals.complete(species), queryFn: () => fetchAllAnimals(species), enabled });
...
export function useAnimalStatusMutation() {
  const client = useQueryClient();
  return useMutation({
    mutationFn: ({ id, status }: { id: string | number; status: string }) => updateAnimalStatus(id, status),
