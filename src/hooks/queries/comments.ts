...
export function useCommentsQuery(postId: string | number) {
  ...
  return useInfiniteQuery({
    ...
    initialPageParam: 0,
    getNextPageParam: (last, pages) => (last.last ?? ((last.content?.length ?? 0) < 20)) ? undefined : pages.length,
  });
}
export function useCommentMutations(postId: string | number) {
  ...
  const create = useMutation({ mutationFn: (payload: CommentDto) => createComment(postId, payload), onSuccess: refresh });
  const update = useMutation({ mutationFn: ({ id, content }: { id: string | number; content: string }) => updateComment(id, content), onSuccess: refresh });
