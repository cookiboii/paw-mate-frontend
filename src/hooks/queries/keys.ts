export const queryKeys = {
  animals: {
    ...
    list: (page: number, size: number, species = 'ALL') => ['animals', 'list', { page, size, species }] as const,
  },
  users: { all: ['users'] as const, me: (session: number) => ['auth', 'profile', session] as const },
  ...
  comments: (postId: string | number, session: number) => ['comments', String(postId), session] as const,
};
