    ...
    <div className={styles.card}>
      ...
      <div className={styles.metricsGrid}>
        <Metric
          ...
          value={result ? result.totalRequests : liveMetrics.length}
          unit="req"
        />
        <div className={styles.metricCard}>
          <div className={styles.metricLabel}>Success Rate</div>
          <div
            className={`${styles.metricVal} ${result?.failureCount ? styles.metricValDanger : styles.metricValSuccess}`}
          >
            {result
              ? Math.round((result.successCount / result.totalRequests) * 100)
              : Math.round(
                  (liveMetrics.filter((metric) => metric.success).length /
        ...
        <Metric
          label="Min / Max"
          value={result ? `${result.minDuration}ms / ${result.maxDuration}ms` : '-'}
          small
        />
      </div>
      {result && (
          ...
          <div className={styles.statusBadgeGroup}>
            {Object.entries(result.statusMap).map(([status, count]) => {
              ...
              return (
                <span
                  key={status}
                  className={`${styles.statusBadge} ${is2xx ? styles.status2xx : is4xx ? styles.status4xx : styles.status5xx}`}
                >
                  {is2xx ? <CheckCircle2 size={14} /> : <AlertCircle size={14} />} HTTP {status} :{' '}
                  {count}건
                </span>
              );
            })}
      ...
      <div className={styles.timelineContainer}>
        <div className={styles.timelineTitle}>
          ...
          <span className={styles.timelineLegend}>🟢 성공 | 🔴 실패</span>
        </div>
        <div className={styles.timelineBars}>
          {metrics.map((metric, index) => {
            ...
            return (
              ...
              >
                <div
                  className={`${styles.barFill} ${metric.success ? styles.barSuccess : styles.barFailed}`}
...
function Metric({
  ...
  return (
    <div className={styles.metricCard}>
      ...
      <div className={`${styles.metricVal} ${small ? styles.metricValSmall : ''}`}>
        {value}
