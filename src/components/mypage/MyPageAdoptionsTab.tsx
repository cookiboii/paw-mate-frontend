...
export default function MyPageAdoptionsTab({ adoptionList }: Props) {
  return (
            ...
            {adoptionList.map((adoption, index) => (
              <li key={adoption.adoptionId || index} className={styles.adoptionItem}>
                ...
                <div className={styles.adoptionInfo}>
                  <h4>
                    {adoption.animalBreed ||
                      adoption.animalName ||
                      '입양 신청 #' + (adoption.adoptionId || index + 1)}
                  </h4>
                  <span
                  ...
                  <div className={styles.timelineWrapper}>
                    <div className={styles.stepBar}>
                      ...
                      <div
                        className={`${styles.stepItem} ${adoption.status === 'PENDING' ? styles.stepActive : styles.stepDone}`}
                      >
                        <div className={styles.stepDot}>2</div>
                        <span className={styles.stepLabel}>신청 심사</span>
                      </div>
                      <div
                        className={`${styles.stepItem} ${adoption.status === 'APPROVED' || adoption.status === 'REJECTED' ? styles.stepActive : ''}`}
                      >
