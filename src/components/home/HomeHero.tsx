...
export default function HomeHero() {
  ...
  const previousSlide = () => setCurrent((previous) => (previous - 1 + images.length) % images.length);
  ...
  return (
    <section className={styles.heroSection}>
      <div className={`${styles.heroContent} animate-slide-up`}>
        <span className={styles.heroBadge}><ShieldCheck size={14} />생명 존중과 책임 있는 입양의 시작</span>
        <h1>한 생명의 평생을 함께할<br />가족을 기다립니다</h1>
        <p>AdoptMate는 안락사 위기의 유기동물들이 안전하고 따뜻한 가정에서 새로운 삶을 시작할 수 있도록 철저한 건강 검진과 투명한 절차를 거쳐 평생 가족을 연결합니다.</p>
        <div className={styles.heroActions}>
          <Link to="/animals" className={`btn-primary ${styles.heroPrimaryBtn}`}>
            <span>보호 중인 동물 확인하기</span><ArrowRight size={18} />
          </Link>
          <Link to="/guide" className="btn-secondary">입양 절차 및 원칙</Link>
        </div>
      </div>
      <div className={styles.heroVisual}>
        ...
        >
          {images.map((image, index) => visibleSlideIndexes.has(index) && (
            <img
              key={index}
              src={image}
          ...
          <button className={`${styles.sliderArrow} ${styles.sliderArrowLeft}`} onClick={previousSlide} aria-label="이전 슬라이드">
            <ChevronLeft size={22} />
          </button>
          <button className={`${styles.sliderArrow} ${styles.sliderArrowRight}`} onClick={nextSlide} aria-label="다음 슬라이드">
