...
export const registerAnimal = async (animalData: AnimalFormData | FormData): Promise<Animal> => {
  ...
  if (!(animalData instanceof FormData) && typeof animalData === 'object' && animalData !== null) {
    ...
    payload = {
      species: (data.species || 'DOG').toUpperCase(),
      breed: data.breed || '',
      color: data.color || '',
      image: data.image || '',
      age: Number(data.age || 0),
      gender: data.gender || 'MALE',
      status: data.status || 'PROTECTED',
    };
  }
...
export const fetchAnimalCursorList = async (
  ...
  return {
    ...data,
    content,
    hasNext: data.hasNext ?? content.length >= size,
    isLast: data.isLast ?? (data.hasNext !== undefined ? !data.hasNext : content.length < size),
  };
};
...
export const fetchAnimalCursorListBySpecies = async (
  ...
  const page =
    lastAnimalId === undefined || lastAnimalId === null || lastAnimalId === ''
      ? 0
      : (cursorPageByLastId.get(key) ?? 0);
  ...
  const hasNext =
    pageData.last !== undefined ? !pageData.last : page + 1 < (pageData.totalPages || 1);
  return {
    content,
    hasNext,
    isLast: !hasNext,
